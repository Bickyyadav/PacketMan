...
const TabbedSidebar = ({ currentWorkspace }: Props) => {
               ...
               <div className='h-full bg-zinc-950 text-zinc-100 flex flex-col'>
                  ...
                  <div className='p-4 border-b border-zinc-800'>
                     <div className='relative'>
                        ...
                        <input type="text" placeholder="Search" className="w-full bg-zinc-900 border border-zinc-700 rounded-lg pl-10 pr-4 py-2 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
                     </div>
                  </div>
                  <div className='p-4 border-b border-zinc-800'>
                  ...
                  </div>
                  {
                     collections && collections.length > 0 ? (collections?.map((collection) => (
                        <div className='flex flex-col justify-start items-start p-3 border-b border-zinc-800 w-full' key={collection.id}>
                           <CollectionFolder collection={{ ...collection, workspaceId: collection.workspaceId as string }} />
                        </div>
                     ))) : (
                        <EmptyCollections />
                     )
   ...
   return (
      <div className='flex h-screen bg-zinc-900'>
         <div className='w-12 bg-zinc-900 border-r border-zinc-800 flex flex-col items-center py-4 space-y-4'>
            {
               sidebarItems.map((item, index) => (
                  <div key={index} onClick={() => setActiveTab(item.label)} className={`w-8 h-8 rounded-lg flex items-center justify-center cursor-pointer transition-colors ${activeTab === item.label
         ...
         <div className="flex-1 bg-zinc-900 overflow-y-auto">{renderTableContent()}</div>
         <CreateCollection isModalOpen={isModelOpen} setIsModalOpen={setIsModalOpen} workspaceId={currentWorkspace.id} />
      </div>
   )
}
